...
def permute(s, table):
    return ''.join(s[i-1] for i in table)
# Circular left shift
...
def encrypt(pt, rkb, rk):
    ...
    for i in range(16):
        ...
        for j in range(8):
            ...
            col = bin2dec(xor_result[j*6+1:j*6+5])
            val = sbox[j][row][col]
            sbox_output += dec2bin(val)
        sbox_output = permute(sbox_output, per)
        result = xor(left, sbox_output)
        left = right
        right = result
        print(f"Round {i+1} {bin2hex(left)} {bin2hex(right)} {rk[i]}")
    final_text = permute(right + left, final_perm)
    return final_text
# --------------------- KEY GENERATION ---------------------
pt = "123456ABCD132536"
...
for i in shift_table:
    ...
    round_key = permute(combined_key, key_comp)
...
rkb.reverse()
rk.reverse()
plain_bin = encrypt(cipher_text, rkb, rk)
plain_text = bin2hex(plain_bin)
